...
// PublicKeySet defines the structure for the JSON file containing public keys, and an endpoint as generated by msg-keygen.
type MessageContactJSON struct {
	Endpoint      string `json:"endpoint"`
	EncryptionKey string `json:"x25519_pub"`  // X25519 public key (binary)
	SignatureKey  string `json:"ed25519_pub"` // Ed25519 public key (binary)
}
...
func LoadContactFromFile(filename string) (MessageContact, error) {
	var contact MessageContactJSON
	...
	xPub, err := base64.StdEncoding.DecodeString(contact.EncryptionKey)
	if err != nil {
		return MessageContact{}, fmt.Errorf("failed to decode X25519 public key: %w", err)
	}
	edPub, err := base64.StdEncoding.DecodeString(contact.SignatureKey)
	if err != nil {
